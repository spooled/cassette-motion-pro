...
namespace CassetteMotionPro.Workspace
{
    public static class FitSessionReportGenerator
    {
        ...
        private static string BuildHtml(ClientRecord client, FitSessionRecord session)
        {
            ...
            AddReportImage(html, "Before", session.BeforeReportImagePath);
            AddReportImage(html, "After", session.AfterReportImagePath);
            html.AppendLine("</div>");
            ...
            html.AppendLine("<div class=\"footer\">Generated by Cassette Motion Pro v0.5.2.</div>");
            html.AppendLine("</div>");
            html.AppendLine("</div>");
            html.AppendLine("</body>");
            html.AppendLine("</html>");
        ...
        private static void AddReportImage(StringBuilder html, string label, string imagePath)
        {
            if (!string.IsNullOrWhiteSpace(imagePath) && File.Exists(imagePath))
            {
                html.AppendLine("<div class=\"media-card\"><img src=\"" + Encode(new Uri(imagePath).AbsoluteUri) + "\" alt=\"" + Encode(label) + " report image\"><div class=\"media-label\">" + Encode(label) + "</div></div>");
                return;
            }

            html.AppendLine("<div class=\"media-card\"><div><strong>" + Encode(label) + "</strong><br><span>Image not added yet</span></div></div>");
        }
