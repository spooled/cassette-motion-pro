...
;General
;--------------------------------
    Name "Cassette Motion Pro"
    OutFile "CassetteMotionPro-${VERSION}.exe"
    
    ; Default installation folder.
    InstallDir "$PROGRAMFILES64\Cassette Motion Pro"
    ...
    InstallDirRegKey HKCU "Software\CassetteMotionPro" "InstallDirectory" 
    BrandingText " "
...
;--------------------------------------
  ...
  !define MUI_LANGDLL_REGISTRY_KEY "Software\CassetteMotionPro"
...
;----------------------------
    ...
    !define MUI_STARTMENUPAGE_REGISTRY_KEY "Software\CassetteMotionPro"
    !define MUI_STARTMENUPAGE_REGISTRY_VALUENAME "Start Menu Folder"
    ;Finish
    !define MUI_FINISHPAGE_RUN "$INSTDIR\CassetteMotionPro.exe"
...
;--------------------------------
	VIAddVersionKey /LANG=${LANG_ENGLISH} "ProductName" "Cassette Motion Pro"
	VIAddVersionKey /LANG=${LANG_ENGLISH} "Comments" "Professional bike fitting and motion analysis"
	VIAddVersionKey /LANG=${LANG_ENGLISH} "CompanyName" "Cassette Fit Studio"
...
Section ""
    ...
    WriteRegStr HKCU "Software\CassetteMotionPro" "InstallDirectory" $INSTDIR

    ; Create uninstaller
    WriteUninstaller "$INSTDIR\Uninstall-CassetteMotionPro.exe"

    ; Register uninstaller to Windows.
    !define AppRemovePath "Software\Microsoft\Windows\CurrentVersion\Uninstall\CassetteMotionPro"
    WriteRegStr HKLM "${AppRemovePath}" "DisplayName" "Cassette Motion Pro"
    ...
    WriteRegStr HKLM "${AppRemovePath}" "Publisher" "Cassette Fit Studio"
    WriteRegStr HKLM "${AppRemovePath}" "UninstallString" "$\"$INSTDIR\Uninstall-CassetteMotionPro.exe$\""

    ; Create shortcuts under all users.
    !insertmacro MUI_STARTMENU_WRITE_BEGIN Application
    SetShellVarContext all
    CreateDirectory "$SMPROGRAMS\$STARTMENU_FOLDER"
    CreateShortcut "$SMPROGRAMS\$STARTMENU_FOLDER\Cassette Motion Pro.lnk" "$INSTDIR\CassetteMotionPro.exe"
    CreateShortCut "$SMPROGRAMS\$STARTMENU_FOLDER\Uninstall Cassette Motion Pro.lnk" "$INSTDIR\Uninstall-CassetteMotionPro.exe"
...
Section "Uninstall"
    ...
    Delete "$INSTDIR\CassetteMotionPro.exe"
    Delete "$INSTDIR\CassetteMotionPro.exe.config"
    ...
    Delete "$INSTDIR\Uninstall-CassetteMotionPro.exe"
    ...
    RMDir /r "$APPDATA\Cassette Motion Pro"

    Delete "$SMPROGRAMS\$MUI_TEMP\Cassette Motion Pro Uninstall.lnk"
    Delete "$SMPROGRAMS\$MUI_TEMP\Cassette Motion Pro.lnk"
    Delete "$DESKTOP\Cassette Motion Pro.lnk"

    ;Remove global shortcuts.
    SetShellVarContext all
    Delete "$SMPROGRAMS\$MUI_TEMP\Uninstall Cassette Motion Pro.lnk"
    Delete "$SMPROGRAMS\$MUI_TEMP\Cassette Motion Pro.lnk"
    ...
    startMenuDeleteLoopDone:
        DeleteRegKey /ifempty HKCU "Software\CassetteMotionPro"
        DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\CassetteMotionPro"
SectionEnd
